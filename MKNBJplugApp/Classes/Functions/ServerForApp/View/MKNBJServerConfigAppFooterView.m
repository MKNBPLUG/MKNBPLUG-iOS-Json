//
//  MKNBJServerConfigAppFooterView.m
...
@implementation MKNBJServerConfigAppFooterViewModel
@end
...
static CGFloat const defaultViewHeight = 350.f;

@interface MKNBJServerConfigAppFooterView ()<UIScrollViewDelegate,
MKMQTTGeneralParamsViewDelegate,
MKNBJUserCredentialsViewDelegate,
MKNBJMQTTSSLForAppViewDelegate>
...
@property (nonatomic, strong)MKNBJMQTTSSLForAppView *sslView;
...
@implementation MKNBJServerConfigAppFooterView
...
- (void)mk_mqtt_generalParams_cleanSessionStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(nbj_mqtt_serverForApp_switchStatusChanged:statusID:)]) {
        [self.delegate nbj_mqtt_serverForApp_switchStatusChanged:isOn statusID:0];
    }
}

- (void)mk_mqtt_generalParams_qosChanged:(NSInteger)qos {
    if ([self.delegate respondsToSelector:@selector(nbj_mqtt_serverForApp_qosChanged:)]) {
        [self.delegate nbj_mqtt_serverForApp_qosChanged:qos];
    }
}

- (void)mk_mqtt_generalParams_KeepAliveChanged:(NSString *)keepAlive {
    if ([self.delegate respondsToSelector:@selector(nbj_mqtt_serverForApp_textFieldValueChanged:textID:)]) {
        [self.delegate nbj_mqtt_serverForApp_textFieldValueChanged:keepAlive textID:0];
    }
}
...
- (void)nbj_mqtt_userCredentials_userNameChanged:(NSString *)userName {
    if ([self.delegate respondsToSelector:@selector(nbj_mqtt_serverForApp_textFieldValueChanged:textID:)]) {
        [self.delegate nbj_mqtt_serverForApp_textFieldValueChanged:userName textID:1];
    }
}

- (void)nbj_mqtt_userCredentials_passwordChanged:(NSString *)password {
    if ([self.delegate respondsToSelector:@selector(nbj_mqtt_serverForApp_textFieldValueChanged:textID:)]) {
        [self.delegate nbj_mqtt_serverForApp_textFieldValueChanged:password textID:2];
    }
}

#pragma mark - MKNBJMQTTSSLForAppViewDelegate
- (void)nbj_mqtt_sslParams_app_sslStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(nbj_mqtt_serverForApp_switchStatusChanged:statusID:)]) {
        [self.delegate nbj_mqtt_serverForApp_switchStatusChanged:isOn statusID:1];
    }
}
...
- (void)nbj_mqtt_sslParams_app_certificateChanged:(NSInteger)certificate {
    if ([self.delegate respondsToSelector:@selector(nbj_mqtt_serverForApp_certificateChanged:)]) {
        [self.delegate nbj_mqtt_serverForApp_certificateChanged:certificate];
    }
}

/// 用户点击选择了caFaile按钮
- (void)nbj_mqtt_sslParams_app_caFilePressed {
    if ([self.delegate respondsToSelector:@selector(nbj_mqtt_serverForApp_fileButtonPressed:)]) {
        [self.delegate nbj_mqtt_serverForApp_fileButtonPressed:0];
    }
}
...
- (void)nbj_mqtt_sslParams_app_clientFilePressed {
    if ([self.delegate respondsToSelector:@selector(nbj_mqtt_serverForApp_fileButtonPressed:)]) {
        [self.delegate nbj_mqtt_serverForApp_fileButtonPressed:1];
    }
}
...
- (void)exportButtonPressed {
    if ([self.delegate respondsToSelector:@selector(nbj_mqtt_serverForApp_bottomButtonPressed:)]) {
        [self.delegate nbj_mqtt_serverForApp_bottomButtonPressed:0];
    }
}

- (void)importButtonPressed {
    if ([self.delegate respondsToSelector:@selector(nbj_mqtt_serverForApp_bottomButtonPressed:)]) {
        [self.delegate nbj_mqtt_serverForApp_bottomButtonPressed:1];
    }
}
...
#pragma mark - setter
- (void)setDataModel:(MKNBJServerConfigAppFooterViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKNBJServerConfigAppFooterViewModel.class]) {
        return;
    }
    ...
    MKNBJMQTTSSLForAppViewModel *sslModel = [[MKNBJMQTTSSLForAppViewModel alloc] init];
...
- (MKNBJMQTTSSLForAppView *)sslView {
    if (!_sslView) {
        _sslView = [[MKNBJMQTTSSLForAppView alloc] init];
