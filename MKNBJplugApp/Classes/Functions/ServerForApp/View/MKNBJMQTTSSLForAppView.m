...
@implementation MKNBJMQTTSSLForAppViewModel
@end

@interface MKNBJMQTTSSLForAppView ()<MKMQTTSSLCertificateViewDelegate>
...
@implementation MKNBJMQTTSSLForAppView
...
- (void)mk_fileSelectedButtonPressed:(NSInteger)index {
    if (index == 0) {
        //CA File
        if ([self.delegate respondsToSelector:@selector(nbj_mqtt_sslParams_app_caFilePressed)]) {
            [self.delegate nbj_mqtt_sslParams_app_caFilePressed];
        }
        return;
    }
    //P12证书
    if ([self.delegate respondsToSelector:@selector(nbj_mqtt_sslParams_app_clientFilePressed)]) {
        [self.delegate nbj_mqtt_sslParams_app_clientFilePressed];
    }
}
...
- (void)sslButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(nbj_mqtt_sslParams_app_sslStatusChanged:)]) {
        [self.delegate nbj_mqtt_sslParams_app_sslStatusChanged:self.sslButton.selected];
    }
}
...
- (void)certificateButtonPressed {
    ...
    [pickView showPickViewWithDataList:dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(nbj_mqtt_sslParams_app_certificateChanged:)]) {
            [self.delegate nbj_mqtt_sslParams_app_certificateChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKNBJMQTTSSLForAppViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKNBJMQTTSSLForAppViewModel.class] || _dataModel.certificate < 0 || _dataModel.certificate > 1) {
        return;
    }
...
- (void)updateSSLButtonIcon {
    UIImage *image = (self.sslButton.selected ? LOADICON(@"MKNBJplugApp", @"MKNBJMQTTSSLForAppView", @"nbj_switchSelectedIcon.png") : LOADICON(@"MKNBJplugApp", @"MKNBJMQTTSSLForAppView", @"nbj_switchUnselectedIcon.png"));
...
- (UIButton *)sslButton {
    if (!_sslButton) {
        ...
        [_sslButton setImage:LOADICON(@"MKNBJplugApp", @"MKNBJMQTTSSLForAppView", @"nbj_switchUnselectedIcon.png") forState:UIControlStateNormal];
