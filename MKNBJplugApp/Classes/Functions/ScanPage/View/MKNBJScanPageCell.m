...
@implementation MKNBJScanPageCell

+ (MKNBJScanPageCell *)initCellWithTableView:(UITableView *)tableView {
    MKNBJScanPageCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKNBJScanPageCellIdenty"];
    if (!cell) {
        cell = [[MKNBJScanPageCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKNBJScanPageCellIdenty"];
    }
    return cell;
}
...
#pragma mark - setter
- (void)setDataModel:(MKNBJScanPageModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKNBJScanPageModel.class]) {
        return;
    }
