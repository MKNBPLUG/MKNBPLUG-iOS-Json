...
@implementation MKNBJMQTTSSLForDeviceViewModel
@end

@interface MKNBJMQTTSSLForDeviceView ()<MKMQTTSSLCertificateViewDelegate>
...
@implementation MKNBJMQTTSSLForDeviceView
...
- (void)mk_fileSelectedButtonPressed:(NSInteger)index {
    if (index == 0) {
        //CA File
        if ([self.delegate respondsToSelector:@selector(nbj_mqtt_sslParams_device_caFilePressed)]) {
            [self.delegate nbj_mqtt_sslParams_device_caFilePressed];
        }
        return;
    }
    if (index == 1) {
        //Client Key File
        if ([self.delegate respondsToSelector:@selector(nbj_mqtt_sslParams_device_clientKeyPressed)]) {
            [self.delegate nbj_mqtt_sslParams_device_clientKeyPressed];
        }
        return;
    }
    //Client Cert File
    if ([self.delegate respondsToSelector:@selector(nbj_mqtt_sslParams_device_clientCertPressed)]) {
        [self.delegate nbj_mqtt_sslParams_device_clientCertPressed];
    }
}
...
- (void)sslButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(nbj_mqtt_sslParams_device_sslStatusChanged:)]) {
        [self.delegate nbj_mqtt_sslParams_device_sslStatusChanged:self.sslButton.selected];
    }
}
...
- (void)certificateButtonPressed {
    ...
    [pickView showPickViewWithDataList:dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(nbj_mqtt_sslParams_device_certificateChanged:)]) {
            [self.delegate nbj_mqtt_sslParams_device_certificateChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKNBJMQTTSSLForDeviceViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKNBJMQTTSSLForDeviceViewModel.class]) {
        return;
    }
...
- (void)updateSSLButtonIcon {
    UIImage *image = (self.sslButton.selected ? LOADICON(@"MKNBJplugApp", @"MKNBJMQTTSSLForDeviceView", @"nbj_switchSelectedIcon.png") : LOADICON(@"MKNBJplugApp", @"MKNBJMQTTSSLForDeviceView", @"nbj_switchUnselectedIcon.png"));
...
- (UIButton *)sslButton {
    if (!_sslButton) {
        ...
        [_sslButton setImage:LOADICON(@"MKNBJplugApp", @"MKNBJMQTTSSLForDeviceView", @"nbj_switchUnselectedIcon.png") forState:UIControlStateNormal];
