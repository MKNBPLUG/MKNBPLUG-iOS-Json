...
static NSString *const fileName = @"MKNBJServerParams.txt";

@interface MKNBJServerParamsModel ()
...
@implementation MKNBJServerParamsModel
...
- (BOOL)checkServerDataProtocol:(id <MKNBJServerParamsProtocol>)protocol {
    if (!MKMQTTValidStr(protocol.host) || protocol.host.length > 64 || ![MKMQTTServerSDKAdopter asciiString:protocol.host]) {
        return NO;
    }
    if (!MKMQTTValidStr(protocol.port) || [protocol.port integerValue] < 1 || [protocol.port integerValue] > 65535) {
        return NO;
    }
    ...
    if (protocol.userName.length > 128 || (MKMQTTValidStr(protocol.userName) && ![MKMQTTServerSDKAdopter asciiString:protocol.userName])) {
        return NO;
    }
    if (protocol.password.length > 128 || (MKMQTTValidStr(protocol.password) && ![MKMQTTServerSDKAdopter asciiString:protocol.password])) {
        return NO;
    }
    ...
    if (protocol.sslIsOn) {
        ...
        if (protocol.certificate == 0 && !MKMQTTValidStr(protocol.caFileName)) {
            //需要根证书
            return NO;
        }
        if (protocol.certificate == 1 && (!MKMQTTValidStr(protocol.caFileName) || !MKMQTTValidStr(protocol.clientFileName))) {
            //双向验证，需要CA根证书、.p12证书
            return NO;
        }
    }
