...
@implementation MKNBJUserCredentialsViewModel
@end

@interface MKNBJUserCredentialsView ()
...
@implementation MKNBJUserCredentialsView
...
#pragma mark - setter
- (void)setDataModel:(MKNBJUserCredentialsViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKNBJUserCredentialsViewModel.class]) {
        return;
    }
...
- (MKTextField *)userNameTextField {
    if (!_userNameTextField) {
        ...
        _userNameTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(nbj_mqtt_userCredentials_userNameChanged:)]) {
                [self.delegate nbj_mqtt_userCredentials_userNameChanged:text];
            }
        };
...
- (MKTextField *)passwordTextField {
    if (!_passwordTextField) {
        ...
        _passwordTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(nbj_mqtt_userCredentials_passwordChanged:)]) {
                [self.delegate nbj_mqtt_userCredentials_passwordChanged:text];
            }
        };
