...
NSString *const MKNBJDeviceModelOfflineNotification = @"MKNBJDeviceModelOfflineNotification";

@interface MKNBJDeviceModel ()
...
@implementation MKNBJDeviceModel

- (void)dealloc{
    NSLog(@"MKNBJDeviceModel销毁");
}
...
- (void)startStateMonitoringTimer{
    ...
    dispatch_source_set_event_handler(self.receiveTimer, ^{
        ...
        if (self.receiveTimerCount >= 62.f) {
            ...
            self.receiveTimerCount = 0;
            self.offline = YES;
            self.state = MKNBJDeviceModelStateOffline;
            dispatch_async(dispatch_get_main_queue(), ^{
                [[NSNotificationCenter defaultCenter] postNotificationName:MKNBJDeviceModelOfflineNotification
                                                                    object:nil
                                                                  userInfo:@{@"deviceID":self.deviceID}];
                if ([self.delegate respondsToSelector:@selector(nbj_deviceOfflineWithDeviceID:)]) {
                    [self.delegate nbj_deviceOfflineWithDeviceID:self.deviceID];
                }
            });
            return ;
        }
...
- (NSString *)currentSubscribedTopic {
    if (ValidStr([MKNBJMQTTServerManager shared].serverParams.publishTopic)) {
...
- (void)updateWithProtocol:(MKNBJDeviceModel *)deviceModel {
